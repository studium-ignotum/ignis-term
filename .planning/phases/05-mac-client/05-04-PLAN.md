...
Output: A working menu bar app that connects to the relay, displays a real session code, shows connection status, tracks IPC sessions, and supports clipboard copy.
</objective>
...
4. **Create tray icon and menu (same as 05-01 but with stored MenuItem refs):**
   - Store references to session_code_item, status_item, count_item in AppState
   - Menu structure same as before

5. **Main event loop with UI updates:**
   ```rust
   loop {
       // Handle tray events
...
3. Verify:
   ...
   - Sessions shows "0"
   - Click "Copy Session Code" then paste somewhere - should be the code
  ...
  <done>
Full integration working: tray shows real session code from relay, status updates on connect/disconnect, clipboard copy works.
  </done>
</task>
...
4. Copy Session Code puts code in clipboard
