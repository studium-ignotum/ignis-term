...
/**
 * Result of attempting to rejoin a session by sessionId
 */
export type RejoinResult =
  | { success: true; session: Session }
  | { success: false; error: 'SESSION_NOT_FOUND' | 'MAC_DISCONNECTED' };
...
export class SessionRegistry {
  ...
  private sessionIdToSession = new Map<string, Session>();
  ...
  createSession(macSocket: WebSocket): Session {
    ...
    this.sessionIdToSession.set(session.sessionId, session);
  ...
  /**
   * Attempt to rejoin a session by sessionId (after page refresh).
   * Validates session exists and Mac is still connected.
   * Replaces existing browser connection if any.
   */
  rejoinSession(sessionId: string, browserSocket: WebSocket): RejoinResult {
    const session = this.sessionIdToSession.get(sessionId);

    if (!session) {
      return { success: false, error: 'SESSION_NOT_FOUND' };
    }

    // Verify Mac is still connected
    if (!session.mac || session.mac.readyState !== WebSocket.OPEN) {
      return { success: false, error: 'MAC_DISCONNECTED' };
    }

    // If there's an existing browser, disconnect it first
    if (session.browser) {
      this.browserToSession.delete(session.browser);
    }

    // Link new browser to session
    session.browser = browserSocket;
    this.browserToSession.set(browserSocket, session);

    // Ensure session doesn't expire while connected
    session.expiresAt = Infinity;
  ...
  removeSession(code: string): void {
    ...
    this.sessionIdToSession.delete(session.sessionId);
  }
